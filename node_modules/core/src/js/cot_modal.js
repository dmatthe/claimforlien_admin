...
$(function () {
  (window['cot_app'] || window['CotApp']).showModal = function (o) {
    ...
    // add a bootstrap class to button
    var returnButton = function(bsType,opt) {
    ...
    var options = $.extend({
      preset:false, // false | 'confirm' | alert
      title: '', //An HTML string. The title of the modal
      body: '', //An HTML string. The body of the modal
      bootstrapType:'default', // primary | success | info | warning | danger
      footerButtonsHtml: returnButton(), //Optional, if unset the modal will have a single Close button. To use, specify an HTML string to render one or more buttons
    ...
    // You must set originatingElement to focus back after modal close
    if (options.originatingElement===null){
      console.error('originatingElement is required');
      return false;
    }
    var template ='';
    // build template regarding preset
    switch (options.preset) {
      case 'alert':
        template =  '<div class="modal fade alert-' + options.bootstrapType + ' '+ options.className + '" id="' + id + '" tabindex="-1" role="dialog" aria-label="Alert" aria-hidden="true" aria-modal="true" aria-describedby="' + id + '_body">' +
      ...
      break;
      case 'confirm':
        template =  '<div class="modal fade ' + options.className + '" id="' + id + '" tabindex="-1" role="dialog" aria-labelledby="' + id + '_title" aria-hidden="true" aria-modal="true">' +
            '    <div class="modal-dialog ' + options.modalSize + '">' +
            '      <div class="modal-content">' +
            ...
            '          </button>' +
            '          <div role="heading" aria-level="2" id="' + id + '_title" class="modal-title">' + options.title + '</div>' +
            '        </div>' +
            '        <div id="' + id + '_body" class="modal-body">' + options.body + '</div>' +
            '        <div class="modal-footer">' +
            returnButton('cancel',{bootstrapType:options.buttons.cancel.bootstrapType,label:options.buttons.cancel.label}) + ' ' + returnButton('confirm',{bootstrapType:options.buttons.confirm.bootstrapType,label:options.buttons.confirm.label})
            '        </div>' +
            '      </div>' +
            '    </div>' +
            '  </div>';
