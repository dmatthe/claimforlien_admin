var cot_map = function (container, o) {
...
// disable/re-enable map dragging when hovering a control box
var thisMap = this.map;
var thisContainer = $('#' + this.container)
thisContainer
  .delegate('.leaflet-control','mouseout', function () {
...
if (!!mapOptions.enableControlBox){
  var Legend = L.control({position:mapOptions.enableControlBox.position});
...
if (!!mapOptions.enableSearchBar){
  thisContainer
    .delegate('.js-getLocationAndDisplay','click',function(e){
      var thisBtn = $(this);
      ...
      getLatLngByKeystring.done(function(data){
        if(data.result.rows.length==1){
          ...
          var popup = L.popup()
            .setLatLng([data.result.rows[0].LATITUDE, data.result.rows[0].LONGITUDE])
            .setContent(data.result.rows[0].KEY_DESC)
            .openOn(thisMap);
  ...
  // add search bar
  var SearchBar = L.control({position:mapOptions.enableSearchBar.position});
  ...
  function searchAllAddress(searchTerm){
  ...
  $('#'+this.container+'_search')
    ...
    $('#'+this.container+'_search')
    .delegate('.resultbutton','mousemove', function(){
      $('.resultbutton').removeClass('isActive');
    })
    $('#searchTerm')
      .on('keydown',function(e){
...
if (!!mapOptions.enableWardSelection){
  var allWards={};
  // add control bar
  var WardsBar = L.control({position:mapOptions.enableWardSelection.position});
  ...
  thisMap.createPane('allWards');
  // wards select event handler
  $('.wardsdiv').delegate('.ward_number','change',function(){
    const selectedArea=$(this).val();
    // remove all wards from the map
    ...
    if(selectedArea!=0){
      ...
      allWards[selectedArea].fl.addTo(thisMap)
  ...
  .where("1=1").run(function(error, wardsData){
    ...
    if($('.ward_number option').length==1){
      // sort allWards
      var order = Object.keys(allWards).sort(function(a,b){return allWards[a].properties.AREA_SHORT_CODE-allWards[b].properties.AREA_SHORT_CODE})
...
L.marker(markerOptions.LatLng, markerOptions.options)
...
cot_map.prototype.addControl = function (o) {
  ...
  var that = this;
  var ctrl = L.control({position: o.position});
