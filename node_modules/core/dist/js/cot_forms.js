...
var cot_form = function cot_form(o) {
...
cot_form.fixClassProperty = function (objectOrArray) {
  $.each($.makeArray(objectOrArray || []), function (i, o) {
    if (o['className'] === undefined && typeof o['class'] === 'string') {
      //this is a hack
      o['className'] = o['class'];
      delete o['class'];
    }
...
var cot_section = function cot_section(o) {
...
var cot_repeatControl = function cot_repeatControl(opts) {
  cot_form.fixClassProperty(opts); // Properties.
  ...
  if (this.initial == null || this.initial < this.min || this.max != -1 && this.initial > this.max) {
...
cot_form.prototype.render = function (o) {
  ...
  $.each(this.sections, function (i, section) {
    if (section.type === 'accordion') {
      ...
      if (!template) {
        ...
        template.innerHTML = ['<div class="panel panel-default accordion__section">', '<div class="panel-heading accordion__header">', '<button class="btn accordion__button" data-type="toggle" aria-controls="{{ID}}" aria-expanded="false"></button>', '</div>', '<div id="{{ID}}">', '{{BODY}}', '</div>', '</div>'].join('');
      ...
      if (section.title || "") {
        ...
        oH3.textContent = section.title; // issue #141 adding an ID to H3 to reference it in input by aria-labelledby
  ...
  var frm = $('#' + this.id);
  var options = $.extend({
    excluded: [':not(.multiselect):disabled', ':not(.multiselect):hidden', ':not(.multiselect):not(:visible)'],
    //exclude all hidden and disabled fields that are not multiselects
    feedbackIcons: {
  ...
  frm.formValidation(options).on('err.field.fv', function (e) {
...
cot_form.prototype.fixFormValidationRender = function (el) {
  el.find('i.form-control-feedback').attr('aria-hidden', 'true'); //this will override feedback icon insertion into a wrong place after the form is rendered
  ...
  $.each(row.fields, function (l, field) {
    switch (field['type']) {
      ...
      case 'daterangepicker':
        ...
        .daterangepicker(field.options).on('show.daterangepicker', function (ev, picker) {
          $($(picker)[0]).focus();
        });
      ...
      case 'datetimepicker':
        $container.find("." + field.id + ".datetimepicker").datetimepicker(field.options).on("dp.change", function () {
      ...
      default:
        if (field.validationtype === "Phone" && IEversion >= 10) {
          ...
          $container.find("#" + field.id + ".phonevalidation, [name$=\"." + field.id + "\"].phonevalidation").each(function (i, el) {
            var $el = $(el);
...
cot_form.prototype.processRepeatControl = function (oRow, oVal, row) {
  ...
  $('.repeatControl-template', $element).addClass(row.setClassName || ''); // Set title.
  ...
  for (var i = 0; i < l; i++) {
    ...
    if (rows[i].fields) {
      ...
      for (var i2 = 0; i2 < l2; i2++) {
        var field = $.extend({}, fields[i2], {
          id: row.id + idxAffix + fields[i2].id
        });
    ...
    } else if (rows[i].repeatcontrol) {// TODO - Repeat control.
    }
  } // Set 'minus' button.
  ...
  $('.repeatControl-set-button-label', $element).text(row.removeBtnLabel || 'Remove row' + (row.title ? ' to ' + row.title : '')); // Control 'plus' button.
  ...
  $element[0].addRow = function () {
    ...
    if (row.max == -1 || $('.repeatControl-sets .repeatControl-set-button .btn', $element).length < row.max) {
      ...
      var $clone = $template.clone().removeClass('repeatControl-template');
      var html = $clone.html();
      ...
      for (var ri = 0; ri < rl; ri++) {
        ...
        for (var fi2 = 0; fi2 < fl2; fi2++) {
          ...
          $('#' + app.id).formValidation('addField', $item, oVal.fields['row[0].' + fields2[fi2].id]); // var validatorOptions = app.validatorOptions(oVal.fields['row[0].' + fields2[fi2].id]);
        ...
        app.fixFormValidationRender($clone); // Add button
        ...
        addButton.onclick = function (oldOnClick, gridDef) {
          return function (e) {
            oldOnClick(e);
            var form = app;
            var $newRow = $('#' + gridDef.id + ' [data-row-index]:last'); // Remove button
      ...
      app.initializePluginsInRow({
        fields: fieldsForPlugin
      }, $clone);
...
cot_form.prototype.processGrid = function (oRow, oVal, row) {
  ...
  var oTR = oTable.appendChild(document.createElement('tr')); //ADD HEADERS
  ...
  oSpan.textContent = "Add/Remove Entries"; //ADD FIRST ROW OF GRID
  ...
  $.each(row.fields, function (l, field) {
    ...
    app.addformfield(field, oFieldDiv); //create a validator specifically for the zero row.
  ...
  oSpan.className = 'sr-only';
  oSpan.textContent = "Remove entry 1"; //ADD GRID TEMPLATE THAT CAN BE USED TO CREATE NEW ROWS
  ...
  oSpan.className = 'sr-only';
  oSpan.textContent = 'Remove entry displayRowNumber'; //Add a 'new' button to the last row
  ...
  oBTN.onclick = function () {
    app[oGrid.id + "-index"]++;
    var rowIndex = app[oGrid.id + "-index"]; //CLONE THE TEMPLATE TO CREATE A NEW GRID ROW

    var $template = $('#' + oGrid.id + '-template');
    var $clone = $template.clone().removeClass('hide').attr('id', oGrid.id + '-row-' + rowIndex).attr('data-row-index', rowIndex);
    var html = $clone.html();
    ...
    $clone.html(html);
    $clone.insertBefore($template); //ADD THE PROPER DELETE FUNCTION TO THE DELETE ROW BUTTON FOR THE NEW ROW
...
cot_form.prototype.processField = function (oRow, oVal, row, field) {
  ...
  var oFieldDiv = oField.appendChild(document.createElement('div')); //LABEL
...
cot_form.prototype.validatorOptions = function (fieldDefinition) {
  var validators = {}; // force field type =='phone'||'email' to have proper validationtype
  ...
  if (fieldDefinition.type === "datetimepicker") {
    ...
    validators.callback = {
      ...
      callback: function callback(value) {
        ...
        return value === '' && !fieldDefinition.required || dates.length === 2 && moment(dates[0], fieldDefinition.options.locale.format, true).isValid() && moment(dates[1], fieldDefinition.options.locale.format, true).isValid();
      }
    };
...
cot_form.prototype.dropzoneFieldRender = function (field) {
  ...
  var cotDropzone = hiddenInput.cotDropzone = new CotDropzone(); // ---------------------------------------------------------------------------

  field.validators = field.validators || {};
  field.validators.callback = {
    callback: function callback(value, validator, $field) {
      var hasError = cotDropzone.dropzone.files.some(function (file) {
  ...
  function updateHiddenInput() {
    ...
    // });
    var value = cotDropzone.dropzone.files;
  ...
  cotDropzone._fillFromModel = function (model) {
    if (field.bindTo) {
      ...
      var files = cotDropzone.initFiles = model.get(field.bindTo) || [];
      ...
      cotDropzone.addInitialFiles.apply(cotDropzone, _toConsumableArray(files));
    }
  }; // ---------------------------------------------------------------------------
  ...
  cotDropzone._watchChanges = function (form) {
    if (field.bindTo) {
      $(hiddenInput).on('change', function (e) {
        if (form._model != null) {
          var newValue = cotDropzone.dropzone.files; // .filter((file) => {
          //   return file.status === Dropzone.SUCCESS || file.status === Dropzone.ADDED;
          // });
  ...
  cotDropzone.uploadAndCallback = function (cbk) {
    ...
    var step1 = function step1() {
      ...
      var filesToUpload = cotDropzone.dropzone.files.filter(function (file) {
        return file.accepted === true && file.status === Dropzone.ADDED;
      });
  ...
  this.success = function (success) {
    return function (event) {
      ...
      function queuecomplete() {
        ...
        if (field.allowFormsubmitionWithUploadError !== false || cotDropzone.dropzone.files.filter(function (file) {
          return file.status === Dropzone.ERROR;
        }).length === 0) {
          if (success != null) {
  ...
  if (field.options.preset != null) {
    var preset = field.options.preset;
  ...
  if (presetInit == null && CotDropzone.defaultOptions != null && CotDropzone.defaultOptions.preset != null) {
    var _preset = CotDropzone.defaultOptions.preset;

    if (typeof _preset === 'string') {
      _preset = CotDropzone.presets[_preset];
    }

    if (_preset != null && _preset.init != null) {
      presetInit = _preset.init;
    }
  }
...
function CotForm(definition) {
  ...
  handleSections(definition['sections'], that.cotForm, ''); // $.each(definition['sections'] || [], function (i, sectionInfo) {
...
CotForm.prototype.setModel = function (object) {
  ...
  this._model = object; // this._model = object;
  // this._fillFromModel(object);
};
...
CotForm.prototype._fillFromModel = function (model) {
  ...
  if (this._isRendered) {
    var handleSections = function handleSections(sections) {
      ...
      for (var si = 0; si < sl; si++) {
        ...
        for (var ri = 0; ri < rl; ri++) {
          if (rows[ri].fields) {
            ...
            // Grid.
            var bindTo = rows[ri].grid.bindTo; // If attribute does not exist.
            ...
            var modelInit = CotForm._getModelInitValue(rows[ri].grid); // If attribute is not a collection.
            // Probably an array.
            ...
            _this.cotForm[rows[ri].grid.id + '-index'] = 0;
            ...
            // Repeat control.
            var bindTo = rows[ri].repeatControl.bindTo; // If attribute does not exist.
            ...
            var modelInit = {}; // If attribute is not a collection.
            // Probably an array.
            ...
            for (var mi = 0; mi < ml; mi++) {
              ...
              for (var r2i = 0; r2i < r2l; r2i++) {
                if (rows2[r2i].fields) {
                  ...
                  var modelG = models[mi]; // If attribute does not exist.
                  ...
                  var modelInit = CotForm._getModelInitValue(rows2[r2i].grid); // If attribute is not a collection.
                  // Probably an array.
      ...
      for (var si = 0; si < sl; si++) {
        ...
        for (var ri = 0; ri < rl; ri++) {
          if (rows[ri].fields) {
            ...
            var modelInit = CotForm._getModelInitValue(rows[ri].grid); // Add button
          ...
          } else if (rows[ri].repeatControl) {// Repeat control.
