...
$(function () {
  ...
  appClass['showTerms'] = function (o) {
    var options = $.extend({
      termsText: 'Content missing: terms text',
      //The body text of the terms and conditions
      disagreedText: 'You need to agree to the terms to proceed',
      //The body text to show when someone disagrees with the terms and conditions
      agreedCookieName: 'terms_cookie' + Math.random().toString().split('.')[1],
      //The name of the cookie to store the user's agreement in
      containerSelector: '',
      //A CSS selector for an element on screen where the terms and conditions will be shown
    ...
    if ($.cookie(options.agreedCookieName) !== "agree") {
      $(options.containerSelector).html(['<section id="cot-terms">', '<form>', '<div class="row">', '<div class="col-xs-12">', '<h2 tabindex="-1">', options.agreementTitle, '</h2>', options.termsText, '</div>', '</div>', '<div class="row">', '<div class="col-xs-12 ">', '<div class="form-group has-feedback">', '<div class="form-control">', '<label>', '<input name="termsAgree" id="termsAgree" aria-required="true" type="checkbox" value="agreed" aria-describedby="termsAgree_errorMessage"> ', '<i class="form-control-feedback glyphicon glyphicon-remove" data-fv-icon-for="termsAgree" aria-hidden="true" style="display: none;"></i>', options.agreeText, '</label>', '</div>', '<div id="termsAgree_errorMessage" class="help-block" role="alert">', '<small class="help-block" style="display: none">', options.disagreedText, '</small>', '</div>', '</div>', '</div>', '</div>', '<div class="row">', '<div class="col-xs-12">', '<div class="btn-group">', '<button type="button" class="btn btn-primary" id="cot-terms-agree">', options.agreeButton, '</button>', '</div>', '</div>', '</div>', '</form>', '</section>'].join(''));
