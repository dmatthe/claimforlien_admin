...
var cot_login = function cot_login(options) {
  this.options = $.extend({
    appName: '',
    //Required, the name of your app, this will be sent to the CC AuthSession API call to login
    ccRoot: '',
    //Optional, defaults to '' (the current protocol and domain will be used), use this to specify the <protocol>:://<domain> to use for the CC AuthSession API call
    ccPath: '',
    //Optional, when specified, this overrides the ccApiPath option of CotSession
    ccEndpoint: '',
    //Optional, when specified, this overrides the ccApiEndpoint option of CotSession
    welcomeSelector: '',
    //Optional, a jquery selector string for the element where the login/logout information should be displayed
    loginMessage: '',
    //Optional, an HTML string to display on the login form, this can be used to explain to the user why they are logging in
    onLogin: function onLogin(cot_login_instance) {//Optional, a function that will be called after the user logs in successfully
    }
  }, options || {});
...
cot_login.prototype._setUserName = function () {
  ...
  if (this.isLoggedIn()) {
    ...
    $el.html("<div class='welcomemsg'><b>User Name</b>: " + this.username + " (<a class='logout' href='#'>Logout</a>)</div>");
    ...
    $el.html("<div class='welcomemsg'><a class='login' href='#'>Login</a></div>");
...
cot_login.prototype._login = function () {
  ...
  var password = passwordInputField.val(); // reset the form status
  ...
  if (!username) {
    ...
    $('.username-group').find('.form-control-feedback').css('display', 'block').removeClass('glyphicon-remove').addClass('glyphicon-ok');
  }
  ...
  if (!password) {
    ...
    $('.password-group').find('.form-control-feedback').css('display', 'block').removeClass('glyphicon-remove').addClass('glyphicon-ok');
  }
  ...
  this.session.login({
    ...
    error: function error(jqXHR, textStatus, _error) {
      console.log('POST Request Failed: ' + textStatus + ', ' + _error, arguments);

      if (_error === 'invalid_user_or_pwd') {
    ...
    always: function always() {
      that.modal.find('.btn').removeAttr('disabled').removeClass('disabled').eq(0).html('Login');
    }
  });
};
...
var CotSession = function CotSession(options) {
  this.options = $.extend({
    appName: '',
    //Required, the name of your app, this will be sent to the CC AuthSession API call to login
    ccApiOrigin: '',
    //By default, login happens on the same origin as the current page. You could override this with, for example, https://was-inter-sit.toronto.ca
    ccApiPath: '/cc_sr_admin_v1/',
    //If needed, you could use a different API path
...
CotSession.prototype.isLoggedIn = function (serverCheckCallback) {
  ...
  if (!serverCheckCallback) {
    ...
    if (!sid) {
      ...
      var url = this.options.ccApiOrigin + this.options.ccApiPath + this.options.ccApiEndpoint + '/' + sid;
      var that = this;
...
CotSession.prototype.login = function (options) {
  options = $.extend({
    ...
    success: function success() {},
    //a function to call after a successful login
    error: function error(jqXHR, textStatus, _error2) {},
    //a function to call after an unsuccessful login
    always: function always() {} //a function to always call after the whole login attempt is complete

  }, options || {});
  var url = this.options.ccApiOrigin + this.options.ccApiPath + this.options.ccApiEndpoint + '?app=' + this.options.appName;
  var payload = {
...
CotSession.prototype.extend = function (minutes) {
  ...
  if (this.sid) {
    this._storeLogin({
      sessionCookieExpiryDate: minutes ? new Date().getTime() + minutes * 60 * 1000 : null,
      sid: this.sid,
...
CotSession.prototype._storeLogin = function (data) {
  ...
  this._cookie('sid', data.sid, {
    expires: date
  });

  this._cookie('cot_uname', data['userID'], {
    expires: date
  });

  this._cookie('email', (data['email'] || '').toLowerCase(), {
    expires: date
  });

  if (data['cotUser']) {
    this._cookie('firstName', data['cotUser']['firstName'], {
      expires: date
    });

    this._cookie('lastName', data['cotUser']['lastName'], {
      expires: date
    });

    this._cookie('division', data['cotUser']['division'], {
      expires: date
    });
