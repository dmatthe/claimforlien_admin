...
var documentReferenceID = void 0;

var getPDF = function getPDF(id) {
  $.ajax({
    url: 'https://was-intra-sit.toronto.ca/downloads(\'\')'
  }).then(function (response) {
    var res = JSON.parse(response.body);
    var filename = 'claimlein-form12__' + documentReferenceID + '.pdf';
    ...
    if (res) {
      ...
      $.ajax({
        url: 'https://was-intra-sit.toronto.ca/c3api_upload/retrieve/claimforlien/' + res.BIN_ID[0].file_name,
        method: "GET",
        type: "GET",
        headers: {
        ...
        success: function success(data) {
          var finalData = data; //file data here
          ...
          if (window.navigator.msSaveOrOpenBlob) {
            ...
            if (linkEXPORT.download !== undefined) {
              ...
              linkEXPORT.setAttribute('href', 'https://was-intra-sit.toronto.ca/c3api_upload/retrieve/claimforlien/' + res.BIN_ID[0].file_name);
...
var getSubmissionSections = function getSubmissionSections(form_id, data) {

  var section = void 0,
      model = void 0,
      registerFormEvents = void 0,
      registerOnSaveEvents = void 0,
      registerPostSaveEvents = void 0,
      totalCharacters = void 0;
  ...
  switch (form_id) {
    case 'Media':
      ...
      registerFormEvents = function registerFormEvents(data) {
    ...
    case 'contacts':
      ...
      registerPostSaveEvents = function registerPostSaveEvents(data) {
        var configName = "contacts.js";
        var dataURL = config.httpHost.app[httpHost] + config.api.get + config.default_repo + "/contacts?$format=application/json;odata.metadata=none&$select=fullName,email&$skip=0&$top=1000&$orderby=id desc&unwrap=true";
        var configURL = config.httpHost.app[httpHost] + config.api.post + config.default_repo + "/Media";
        var appName = config.default_repo;
        var config_content = {};
        var doc_id = data.id;
        $.ajax({
          "url": dataURL,
          ...
          updateConfig(configURL, configName, "application/javascript", "var sys_config = " + JSON.stringify(config_content) + "", Cookies.get(config.default_repo + ".sid")).then(function () {
    ...
    case 'submissions':
      section = [{
        ...
        rows: [{
          ...
          // },
          {
            ...
            options: {
              url: 'https://was-intra-sit.toronto.ca/c3api_upload/upload/claimforlienP/attachments',
              ...
              init: function init() {
                ...
                this.on("removedfile", function (file) {
                  ...
                  this.getRejectedFiles().map(function (file) {
      ...
      registerFormEvents = function registerFormEvents(data) {
        ...
        //   //   grecaptcha.execute('6LeN_XIUAAAAAEd8X21vFtkJ3_c7uA0xpUGcrGpe').then(function(token) {

        //   //     $.ajax({
        //   //       //url: `undefined`,
        //   //       url: 'https://was-intra-sit.toronto.ca/c3api_data/v2/DataAccess.svc/cot_dts_recaptcha/app_config/ca.toronto.api.dataaccess.odata4.verify',
        //   //       method: "POST",
        //   //       type:"POST",
        //   //       headers:{
        //   //         'Content-Type':'application/json',
        ...
        $('.btn-addFiles').remove();
        $('#js-download-button').click(function (evt) {
          ...
          // var url = `https://was-intra-sit.toronto.ca/c3api_upload/retrieve/claimforlien/${res.BIN_ID[0].bin_id}`;
          // a.href = url;
        ...
        registerOnSaveEvents = function registerOnSaveEvents(data) {
 ...
 */
var getColumnDefinitions = function getColumnDefinitions(formName, filter) {
  ...
  switch (formName) {
    case 'Media':
      columnDefs = [{
        ...
        render: function render(data, type, row, meta) {
          var desc = "Open " + config.formName[formName] + " " + row[config.formHeaderFieldMap[formName]];
          var view_button = "<button aria-label='" + desc + "'class='btn btn-sm btn-default view_btn'>Open</button>";
          return view_button;
        }
    ...
    case 'contacts':
      ...
      }, { "data": "fullName", "title": "Full Name", "filter": true, "type": "text" }, { "data": "email", "title": "Email", "filter": true, "type": "text" }];
      view = "contacts";
      break;
    case 'submissions':
      columnDefs = [{
        ...
        render: function render(data, type, row, meta) {
          var desc = "Open " + config.formName[formName] + " " + row[config.formHeaderFieldMap[formName]];
          var view_button = '<button aria-label="' + desc + '" class="btn btn-sm btn-default view_btn">' + data + '</button>';
          return view_button;
        }
      }, {
        ...
        render: function render(data, type, row, meta) {
          var desc = "Open " + config.formName[formName] + " " + row[config.formHeaderFieldMap[formName]];
          var view_button = data; //`<button aria-label="${desc}" class="btn btn-sm btn-link view_btn">${data}</button>`;
          return view_button;
        }
      }, { "data": "name_of_owner", "title": "Name of Owner", "filter": true, "type": "text" }, { "data": "amount_claimed", "title": "Amount Claimed", "filter": true, "type": "text" }, { "data": "location_premises", "title": "Premises Address", "filter": true, "type": "text" }, { "data": "short_description", "title": "Short Description", "filter": true, "type": "text" }, { "data": "contract_number", "title": "Contract Number", "filter": true, "type": "text" }, {
        "data": "__CreatedOn",
        "title": "Created Date",
        "filter": true,
        "type": "datetime",
...
var registerEvents = function registerEvents() {
  ...
  var cur_user = getCookie(config.default_repo + '.cot_uname') && getCookie(config.default_repo + '.cot_uname') !== "" ? getCookie(config.default_repo + '.cot_uname') : "not set";
  ...
  $("#maincontent").off('click', '.tablink').on('click', '.tablink', function () {
    ...
    var newRoute = $(this).attr('data-id') + '/?ts=' + new Date().getTime() + '&status=' + $(this).attr('data-status') + '&filter=' + $(this).attr('data-filter');
  ...
  $(".form-control-clear").hide($(undefined).prev('input').val());
};
...
var welcomePage = function welcomePage() {
  ...
  var welcome_template = config.dashboard_template;
...
var appInitialize = function appInitialize() {
  console.debug('appInitialize');

  var BasicSearchDefinition = function BasicSearchDefinition() {
    var formId = 'js-basic--search';
    return {
      id: formId, //required, a unique ID for this form
      //title: '', //optional, a title to display at the top of the form
      rootPath: '/webapps/claimforlien_admin/', //optional, only required for forms using validationtype=Phone fields
      success: function success(evt) {
        ...
        filterStr = filters.map(function (filter) {
          return filter;
        }).join(' and ');
        window.location.href = '/webapps/claimforlien_admin/#submissions/?&status=&filter=' + filterStr;
  ...
  var AdvancedSearchDefinition = function AdvancedSearchDefinition() {
    var formId = 'js-search--advanced';
    return {
      id: formId, //required, a unique ID for this form
      //title: '', //optional, a title to display at the top of the form
      rootPath: '/webapps/claimforlien_admin/', //optional, only required for forms using validationtype=Phone fields
      success: function success(evt) {
        ...
        if (query) {
          ...
          window.location.href = '/webapps/claimforlien_admin/#submissions/?&status=&filter=' + filter;
        }
          ...
          repeatControl: {
            ...
            rows: [{
              fields: [{
                ...
                title: 'Field Name',
                type: 'dropdown',
                choices: [{ text: 'Lien ID', value: 'id--text' }, { text: 'Name of Lien Claimant', value: 'name_of_lien_claimant--text' }, { text: 'Name of Person Supplied', value: 'claimant_supplied--text' }, { text: 'Premises Address', value: 'location_premises--text' }, { text: 'Premises Description', value: 'description_of_premises--text' }, { text: 'Address for Service', value: 'address_for_service--text' }, { text: 'Name of Owner', value: 'name_of_owner--text' }, { text: 'Address of Owner', value: 'owner_address--text' }, { text: 'Address of Person Supplied', value: 'claimant_supplied_address--text' }, { text: 'Supplied Date From', value: 'supplied_from--date' }, { text: 'Supplied Date To', value: 'supplied_to--date' }, { text: 'Short Description', value: 'short_description--text' }, { text: 'Contract Price', value: 'contract_price--text' }, { text: 'Amount Claimed', value: 'amount_claimed--text' }, { text: 'To the Claim of', value: 'to_the_claim_for_lien_of--text' }, { text: 'Contract Number', value: 'contract_number--text' }, { text: 'Owner Contact', value: 'contact_person--text' }, { text: 'Signature Date', value: 'date--date' }]
              }, {
                id: 'filter_by',
                title: 'Filter By',
                type: 'dropdown',
                choices: [{ text: 'Equals To', value: '' }, { text: 'Contains', value: '' }, { text: 'Does Not Equal To', value: '' }]
              }, {
                id: 'search_term',
                title: 'Search Term',
                type: 'text'
...
var updateConfig = function updateConfig(entitySet, name, contentType, media, sid) {
  ...
  step1(entitySet, name, contentType, media, sid).then(function (data, textStatus, jqXHR) {
    step2(entitySet, name, contentType, media, sid).then(function (data, textStatus, jqXHR) {
      step3(entitySet, name, contentType, media, sid).then(function (data, textStatus, jqXHR) {
        step4(entitySet, name, contentType, media, sid).then(function (data, textStatus, jqXHR) {
          defer.resolve(data, textStatus, jqXHR);
        }, function (jqXHR, textStatus, errorThrown) {
...
var patchConfig = function patchConfig(dataURL, configURL, appName, configName, targetObject, successCallBack, processor) {
  ...
  $.when($.ajax({
    ...
    updateConfig(configURL, "site_variables.json", "application/json", payload, Cookies.get(config.default_repo + ".sid")).then(function () {
